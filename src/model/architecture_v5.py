"""V5 CNN + Transformer architecture with explicit constraint residual channels.

V5 keeps the V1_5 feedback loop, then adds rule-derived channels computed from
the visible board and the previous mine probability estimate. The extra channels
make local number-constraint imbalance directly visible to the network.
"""
...
class MinesweeperTransformerV5(nn.Module):
    """V1_5-style explicit feedback plus rule-derived constraint channels."""
    ...
    def load_pretrained(self, checkpoint_path: str, device: str = "cpu") -> None:
        """Load V5 or migrate V1/V1_5-style checkpoints by zero-padding channels."""
