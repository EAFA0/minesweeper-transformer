...
# 2. 核心路线（3 阶段，默认 --refine 8）
python scripts/train_stage.py --stage S1    # 8×8/10雷（1 epoch）
python scripts/train_stage.py --stage S2    # 8×8/20雷（1 epoch）
python scripts/train_stage.py --stage S3    # 10×10/40雷（2 epoch）

# 3. 评估（自适应 refinement + board pool 自动开启）
python scripts/evaluate.py checkpoints/S3/best_model.pt --width 10 --height 10 --mines 40
...
|------|------|----------|------------|
| S2 (refine 8) | 8×8/20 | ~93% | 0.999 |
| S2.5 (refine 5) | 8×8/25 | 99.6% | 1.000 |
| S2.5 (refine 5) | 10×10/40 | **97.0%** | 0.999 |
| S3 (无 refine) | 16×16/40 | 99.0% | 1.000 |
...
- 混合数据：`python scripts/generate_data.py --mixed --n_samples 12000`
