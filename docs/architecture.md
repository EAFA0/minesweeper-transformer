...
当前结果：
- `v5_replay_S5_denoise_rank` 在 S5 8×8/32 固定 500-board cache 上裸模型达到 490/500 WR = 98.00%
- `v5_replay_S5_denoise_rank_ft2` 在 `--rule_guard --refine_steps 5` 下达到 498/500 WR = 99.60%
- `v5_replay_S5_denoise_rank_ft2` 在 `--rule_guard --prob_zero_guard --refine_steps 5` 下达到 1000/1000 WR = 100.00%

### Evaluation Guard Strategy (2026-06-12)

当前 100% 成绩不是纯裸模型成绩，而是“模型 + 保守可证明安全辅助框架”：

```text
model refinement -> argmin P(mine)
rule_guard       -> ConstraintSolver safe cells first
prob_zero_guard  -> ProbabilitySolver only when it finds P(mine)=0 cells
```

边界：
- `rule_guard`: 只在 `ConstraintSolver` 找到可证明 safe cells 时限制候选集；候选内部仍由模型排序。
- `rule_mine_guard`: 只做诊断，虚拟排除 solver-proven mines，不真实插旗；最终 100% 路径不依赖它。
- `prob_zero_guard`: 只在 `rule_guard` 无 safe cells 时调用 `ProbabilitySolver`，并且只接管 `P(mine)=0` 的动作；无零概率格时继续用模型。

采用该组合的原因：
1. `ConstraintSolver` 能修复大量基础规则排序抖动，但在某些局面只能证明 mines 或完全找不到 safe cells。
2. 全局 ProbabilitySolver fallback 会在大组件 fallback `0.5` 时破坏模型排序，因此不能直接替代模型。
3. `prob_zero_guard` 只使用零概率安全格，相当于将 no-guess benchmark 的“存在可证明安全动作”作为评估侧保险，不把模糊概率排序外包给 solver。
...
- 推理: 默认 `eval_max_steps=4` (与训练步数对齐)，通过 `max|ΔP| < 1e-2` 早停；当前 100% S5 组合显式使用 `--refine_steps 5`
