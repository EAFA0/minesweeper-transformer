...
### 决策
两条路线并存，互不冲突:
1. **分阶段课程** (`train_stage.py`): S1(8×8/10) → S2(8×8/15) → S3(8×8/20) → S4(8×8/25)，每阶段继承权重
...
2. 阶段名只使用 `S1` 到 `S4`，不再引入 `S1.5`/`S2.5` 等历史密度名
3. 统一入口 `train_stage.py --stage S1/S2/S3/S4` 或 `--recipe v5_curriculum`，消除“该跑哪个阶段”的困惑
