...
### 当前局限 (实现偏差)
- **缺失残差**: 目前 `mem_state` 在迭代间没有外部残差连接 ($m_{t+1} = \text{Transformer}(m_t)$)，存在状态漂移风险。
- **缺失 Grounding**: CNN 特征没有在每一轮迭代中重新注入。
...
- **收敛检测推理**: 推理阶段使用 `eval_max_steps=4` (当前与训练一致)，通过 `max|ΔP| < 1e-2` 进行早停。
...
### 当前风险
- **训练/推理步数解耦不足**: 目前 `eval_max_steps` 被限制在 4 步，无法充分发挥 Deep Inference 的潜力。
- **BPTT 梯度消失**: 在没有残差连接的情况下，梯度在 4 步迭代中可能迅速消失。
